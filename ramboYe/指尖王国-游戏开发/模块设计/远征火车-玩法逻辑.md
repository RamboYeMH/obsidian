...
> | 文件 | 职责 |
> |---|---|
> | `ctl_exptrain.go` | 核心玩法：燃料、驻防、攻防、发车、瞭望塔、位置查询 |
> | `ctl_exptrain_reward.go` | 奖励与结算：进度奖、贡献奖、排名结算、活动协议 |
> | `ctl_exptrain_cmd.go` | GM 命令 |
...
| `TET_Process` | 燃料进度攒满 `MaxCount` | `CheckTrainStart`（`ctl_exptrain.go:126`），同时取消停留定时器 |
...
主入口 `PTroopTransFuelToTrainBuildArrive`（`ctl_exptrain.go:87`）。另有三个加进度 / 加贡献的口子：

- `PTroopDefendExpTrainBuild`（`ctl_exptrain.go:227`）：驻防时身上带的燃料也直接算进度；
...
| 环节 | 函数 | 要点 |
|---|---|---|
| 驻防校验 | `PTroopDefendExpTrainBuildCheck`（`ctl_exptrain.go:173`） | 同阵营、人数上限 `DefenseMember`、一人一队 |
| 驻防 | `PTroopDefendExpTrainBuild`（`ctl_exptrain.go:227`） | 进 `Defenders`，坐标贴到车站 |
| 进攻校验 | `RTroopAtkExpTrainArriveCheck`（`ctl_exptrain.go:278`） | 禁同阵营、联盟集结数上限 `ExpeditionTrainRallyMax`、`ProtectEndTs` 保护期 |
| 进攻结算 | `RTroopAtkExpTrainArrive`（`ctl_exptrain.go:306`） | 有守方集结队直接打；没有则把散装 `Defenders` **现场合并成一支 `RallyTroop`**；一个人都没有时造一支 `owner=0` 的空队顶上，保证战斗流程不卡 |
| 战败掉落 | `ExpTrainDefendEnd`（`ctl_exptrain.go:409`） | 按 `ExpTrainDropPositionCfg` 在车站周围撒燃料采集物（`CreateFuelGatherItem`），每次扣 `ExpeditionTrainTaskItemDrop[1]`，受 `TodayDropCount` 每日次数限制；随后进入 `ExpeditionTrainProtectTime` 保护期，发跑马灯 + 阵营邮件 |
| 编队阵亡掉落 | `PlayerTroopDeadCheckDropFuelGatherItem:609` | 运输途中被杀，燃料原地掉落 |
| 跨天重置 | `OnEventExpTrainNextDay`（`ctl_exptrain.go:499`） | 重置各阵营 `TodayDropCount` |
| 放弃驻防 | `GiveupDefendExpTrain`（`ctl_exptrain.go:708`） | 车站还在则从建筑移除；火车已在行进中则从 `train.DefenderIds` 移除（`ExitExpTrain`） |
| 阵营预警 | `CampWatchTower` / `campWatchTowerCampId`（`ctl_exptrain.go:1117`） | 火车建筑不做敌对关系判断，阵营中心类建筑才判断 |
...
ExpAddGamePlayNum(area, num)              ctl_battlefield_game_play.go:1084
  ...
  · 全部任务完成 → ExpGamePlayFinish
       ...
       → ExpTrainOnGamePlayFinish          ctl_exptrain.go:932
...
| 进度奖励补发 | `SendTrainBuildUnRecvRewards`（`ctl_exptrain_reward.go:237`），发车时对 `PlayerRecords` 中未领取者发邮件 | `ExpeditionTrainTaskMail` |
...
1. **`PTroopDefendExpTrainBuild` 删错道具**
...
2. **阵营排名奖发不出去**（`OnActvExpTrainEndNtf` 与 `cmdSendExpTrainRankRewards`）
...
3. **`OnExpTrainPlayerContributeRewardReq` 回包数值翻倍**
...
4. **发车补发奖励跨玩家串档**（`SendTrainBuildUnRecvRewards`）
   已达进度的档位集合 `ids` 是所有玩家共享的一份 map，循环里对已领玩家执行 `delete(ids, ...)`，把档位从公共集合里永久删掉。结果：排在前面的玩家已领过的档位，后面的玩家再也补发不到。
   已改为每个玩家独立判断 `tb.ProcessRewards[pid]`，并对 `railwayCfg.Reward[index]` 加越界保护。

5. **驻防带入的燃料不计入贡献者名单**（`PTroopDefendExpTrainBuild`）
   该路径加了 `Process` 却没写 `PlayerRecords`，发车补发奖励时会漏掉这些玩家。抽出 `addTrainFuelProcess` 后两条路径统一记录。

### 重构（2026-08-06）

按职责拆成三个文件，抽出公共函数消除重复：

| 函数 | 作用 | 合并前 |
|---|---|---|
| `takeTroopFuel` | 从编队摘燃料 | 3 处 |
| `addTrainFuelProcess` + `broadcastTrainFuelAdd` | 加进度/贡献/发车检查/广播 | 2 处 |
| `settleTrainRankRewards` | 阵营排名结算 | 2 处 |
| `getTrainDefenderTroops` + `rangeTrainDefenderWatchTowers` | 驻防编队遍历 | 2 处 |
| `cancelTrainTimer` / `removeTrainBuild` / `removeTrainMonster` / `newTrainBuildAt` | GM 与到站的车站增删 | 4~6 处 |
| `getCmdCampTrain` | GM 命令解析阵营火车 | 8 处 |
| `mergeTrainDefendersToRally` / `newTrainDefenderRally` / `newEmptyTrainDefenderRally` | 拆分 93 行的 `RTroopAtkExpTrainArrive` | — |

顺带补的健壮性：`GetExpeditionCampTrain()` 直接取 `.Trains` 的空指针风险、`railwayCfgs[0]`/`[len-1]` 与 `args[1]` 的越界、`secCfg.Reward[0]` 越界、GM 命令里 `areaBuild`/`trainBuild` 为 nil 仍继续用。

### 仍存在的可优化点

- “从 `GotAssets` 摘取燃料”的逻辑在 **4 处**重复（`PTroopTransFuelToTrainBuildArrive`、`PTroopDefendExpTrainBuild`、`PlayerTroopDeadCheckDropFuelGatherItem`、`PTroopTransFuelToAreaBuildArrive`），建议抽成 `PlayerTroop.TakeFuel(fuelId) int64`。
