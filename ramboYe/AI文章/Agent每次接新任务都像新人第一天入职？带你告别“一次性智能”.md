...
source: "https://mp.weixin.qq.com/s/AzDms1atG5mRw7vbDnm7Qg"
author:
  - "[[田倬月]]"
published:
created: 2026-09-05
...
tags:
  - "clippings"
---
田倬月 腾讯云开发者 *2026年9月3日 08:45*

关注腾讯云开发者，一手技术干货提前解锁👇

![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/ZRhjO8xAWr5cVljsZaTerseHo0b30q5ACI8PKQZDgtq7iaRpXwCrV8UnWMvP2gXatJwwmsic6wcF404wPNYIBa0PmibGC70MSZISib2VJoD3JRk/640?wx_fmt=png&from=appmsg&tp=webp&wxfrom=5&wx_lazy=1#imgIndex=0)
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/ZRhjO8xAWr6cLfj08PgIdgnk9PTtNsprIcdzYKuTo6DMdxuzxdEia9MtiannnJbIMibeNRAtcNr8YicKYoWKicdTmhZfGtBNBoX0Aa1FR2f8jicr0/640?wx_fmt=png&from=appmsg#imgIndex=3)
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/ZRhjO8xAWr4xbkDII7O0E0gjaz7mpIp6ian8GMSYueCo6B8ePlxrZBibnIVO84Yn096LpRibCTNl05H6922rq23aULt2U8Myb5FmbJic01ngSbw/640?wx_fmt=png&from=appmsg#imgIndex=4)
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/ZRhjO8xAWr7ibjn55OFohx4NmtFSiacH0MAe4I1kwkudROuZLUoyBfLu5YczOTZJ7aPu6LJQibLKSaiaiavbS7YNzpqOQXOxsRObZvKd2icdic8xxc/640?wx_fmt=png&from=appmsg#imgIndex=5)
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/ZRhjO8xAWr6aiblVYKyPIMs5ca2UlCyYSMp7vgrKdJYOibWNkjoG1J3pt4owOp3YCrcibn5aMYEyWoL6EGdWLh3NjUAR5FnFDpduIWicD0Fg94U/640?wx_fmt=png&from=appmsg#imgIndex=6)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/ZRhjO8xAWr6fKHbQFoL1bA1DUqXtwYOtriaW7V4huk4vUNCgUrmhSqwyRWPrIKmRuHoQTbPZGS3gCrbQqrUsAfumtTUkicG3QbYZMhhn4Jic08/640?wx_fmt=png&from=appmsg#imgIndex=7)
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/ZRhjO8xAWr50XfP9mHcAHpgDqwuChB4A4zMhDFC472l5R7n7oB2WjYwGTQTVT9egicNjMgBtROiaZd34dibbWiavwpSv3nsHVoyzultSgx20z7E/640?wx_fmt=png&from=appmsg#imgIndex=8)

**2.5 最终形成的整体架构**

经过前面几轮收敛，Forge Memory 形成了下面这组责任边界。

![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/ZRhjO8xAWr4XmBzFAUBUMAFwJ06SicxEClv9kbJz1fWMU5JRMkML2k5LoHBynXZaRxIOpvB1EmTSpvica0lKqB6rvszYs9kAMq0ribVDNfG0Ik/640?wx_fmt=png&from=appmsg#imgIndex=9)

项目仓库保存任务协议、当前代码和验证结果；知识数据边界保存任务线索、知识对象与派生索引。AI 负责知识的理解、判断与持续维护，确定性执行层负责路由、写入与校验，Git 与人工审阅承担高风险团队事实的确认责任。

各部分各管一件事：

- **项目仓库** 保存当前能够运行和验证的事实——代码、测试、任务协议。它回答“系统现在怎样工作”；
- **知识空间** 保存代码难以表达的工程判断——适用边界、设计理由、排障经验和修正历史。稳定的 ID 与索引让这些经验能够跨任务被找到、更新和纠正；
- **AI** 是知识的理解者和维护者：判断什么与当前任务相关、什么值得保留，并根据新的代码事实和任务结果持续更新、合并、纠正或淘汰已有知识；
- **确定性执行层** 是 AI 维护知识空间的工具层：提供读取、路由、校验、写入、索引更新和状态迁移等确定性能力，把语义判断转换成可执行、可检查的结构操作；
- **Git 与人工审阅** 承担治理责任：高风险约束和团队级事实需要经过人工确认，才能进入默认推荐或长期生效。

通过分层，代码事实与知识空间承担不同的职责：代码事实描述系统现在如何工作，知识空间记录团队为什么这样做、遇到过什么问题，以及下一次应该如何判断。AI 持续连接两类信息，并随着任务推进维护知识空间，使已有经验能够跟随代码和项目状态一起演进。

![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/ZRhjO8xAWr70Y27r0j5ROojNYhaPo3t20KjnB7LckZ2SPQwhQia2gPZQL6iadqVSfndt5KJF1DRC4ibiacTvg0mJuCnWJib9DMhaUF11tZK2V4s8/640?wx_fmt=png&from=appmsg#imgIndex=10)
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/ZRhjO8xAWr5XE2x0vwBibia2XyXXjDpVs6hQIEd5Dy0Y8TVr4bkh1Tkcz5e1ggfI7TicxdDicOia7PN8r9rK5IFibf5pushNBe6n9WztVbL8Eic2iao/640?wx_fmt=png&from=appmsg#imgIndex=11)
...
```makefile
id: D001type: decisionknowledge_role: rationalesummary: 协议标签继续使用当前解析器，不跨分包复用另一套富文本实现scope: protocol-labelrelated_files:  - pkgAsync/components/protocol-label/parseProtocolHtml
```

**3.2 先决定是否进入长期知识，再决定如何影响 Agent**
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/ZRhjO8xAWr55Jr4GCbnFibHtVcFDgeRYrDbWqaCDdYqQpLkUhPJWyYQcFxrOLBpjcKfJpTFegqdA2tjvJDOUTKfod7Wfn8vp8my1bFTY22Mo/640?wx_fmt=png&from=appmsg#imgIndex=12)
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/ZRhjO8xAWr5gnASvz4quqlTa4wYGAAJO4Q4CExQUibu4iaXMKSiaNiafqKlczDI6mzBpic6cMXAjc8eR606LVLxTV7VXI6kEa1Yw3Q99ZicZibp3DU/640?wx_fmt=png&from=appmsg#imgIndex=13)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/ZRhjO8xAWr7y1MB9xZAGW1eQeSAVDjtRiboo0ibsW1h6ryEvQAUW4gUO5Eib6nUqI9EeBIWQnlnlvicl1VCmnV6Pd41qx7KQZT21Yre7Bw54pCw/640?wx_fmt=png&from=appmsg#imgIndex=14)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/ZRhjO8xAWr6YzdETCibictuuWSWOGul5PYqiaf867ZSqIB5dKDVhfaonQQGUHB4onQKFQRpIV87icw1Dsng4tXSf1ZsjgKia5M8qULclF9LuNiarU/640?wx_fmt=png&from=appmsg#imgIndex=15)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/ZRhjO8xAWr5MOV9DvyGZRGCw16w6EldA1wk1h1oJErYOUtsHGiaVmWRA3RKMiaYBEGqnqVMR0RZOXRXfFccJSRViaOU4vYqEg1eXC3bInVdh4E/640?wx_fmt=png&from=appmsg#imgIndex=16)

读取准入的第一道判断不是相关性，而是 **消费资格** 。一条候选对象即便主题与任务高度相关，也不一定应该进入 Agent 视野：未完成的骨架不进入正常推荐，明确停用的知识退出正常消费，结构不完整的对象不能作为有效知识，未经人工确认的约束规则在确认前不会成为默认边界。这些资格判断是纯确定性的一段代码，AI 不参与。约束规则单独走一条更严格的资格链，是因为它和其他五种角色影响 Agent 的方式不同：导航知识和模型知识主要影响 Agent 的探索路径与系统理解，进入任务后仍会接受当前代码核对；约束规则则会更早地缩小候选方案空间，一旦错误生效，可能让合理方案从讨论开始就被排除。

通过资格门以后，系统再依据当前任务已经暴露出的文件、模块、符号、概念和摘要关键词等事实信号缩小范围，达到相关性门槛的对象进入有限的 **推荐候选** ，其余保留在 **知识目录** 中等待任务途中继续下钻。角色只在候选相关性接近时帮助稳定排序，不替代文件、模块和概念等当前任务事实。一条知识如果没有获得正向相关性证据，不会仅仅因为角色重要就被塞进推荐集合。这一点与前一段的资格判断合起来，构成了整个读取准入的骨架：
...
第一件事，是知识怎样真正进入主任务。这里要做的是避免把整篇正文塞回主 Agent 的上下文：一是浪费预算，二是主 Agent 并不需要理解知识对象的内部协议，它需要的是这条历史判断对当前任务意味着什么。记忆读取因此发生在一个 **独立子流程** 里——记忆子流程在自己的上下文中打开正文，判断这条知识是否与当前任务真正相关，再把提炼后的 **知识作用** 返回主 Agent。主 Agent 收到的不是"某模块隔离知识的第 3 段"，而是一句更接近工程判断的表述，例如：“在采用跨模块复用之前，先重新确认当前依赖边界。”
...
沉淀准入 **默认不写入** 。一次任务有新发现，不代表长期知识应该增加。判断一条候选是否有资格改变长期知识，按顺序问五个问题：
...
4. 是否已经完整到足以让下一次任务理解？
5. 长期知识里是不是已经存在同一个判断？
![图片](https://mmbiz.qpic.cn/mmbiz_jpg/ZRhjO8xAWr7YtlDRXJwiaIR1B1ibSeibPeLTyGJUR8pib2bUzm8lWtVlznsgkXLJT5wy1pg1A9FJgIZsCbwwERoo7TEKvgdicicp4uGMxNph1C7KA/640?wx_fmt=jpeg#imgIndex=17)

这五个问题共同得到三个结果，优先级固定：

- 没有长期价值，或事实可以从当前代码低成本恢复 → **不沉淀** ；
- 长期层里已经存在同一判断的对象 → **更新已有知识** ；
- 出现了独立的新判断，且证据、边界、失效条件都足够完整 → **新建知识** 。
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/ZRhjO8xAWr7TddYCWyTb8HenQ3zibdMlABadkFCJFf0L2Xqican14mANPulOYBLb0DZuuP8wP1WpMkcaXCCutyh1G3aMu6uQwct0E7BLUIQek/640?wx_fmt=png&from=appmsg#imgIndex=18)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/ZRhjO8xAWr5OvBO4pLQgsSxnSAbdx3s96GhXibD1b8L4EdawyYaiagLoTTvCJ1Z9XAy806RyduKN1ia7sibY51toicrhrBGLgiaic7FyMjLmv2DjsI/640?wx_fmt=png&from=appmsg#imgIndex=19)
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/ZRhjO8xAWr71BvV5hZMFjsGPTUsTTzRO7QFngv2V3tpJMJK2J0Uy5El6l51b0H6EmTVpQic0WvcOt2uQVhMYo6t1fKt5g6034fV7ePynbNqU/640?wx_fmt=png&from=appmsg#imgIndex=20)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/ZRhjO8xAWr7rDbPlWJibnVjYbic8XAM1ALp7xPM87uHfiaCiauqx8XdlIickUfQzNgQnN6DFFHIKnr9tkZn0Viaw2MzFgZ3msKpFqmVpw4osibzNls/640?wx_fmt=png&from=appmsg#imgIndex=21)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/ZRhjO8xAWr5C6p8ia6coicma1TAKe9d1h0dqf3qlicDRic5ibTLYjoaUR9OTk1BpiboF3uibJ2eG8EIb1OASwzwxDvzveia4VnMnODCL7nUYBVNzoks/640?wx_fmt=png&from=appmsg#imgIndex=22)
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/ZRhjO8xAWr7PD2ZNyQCI8IwPqBiaJVP4N6WNrbTpib6tMawibiaCGzxt2roC1byXwTQ8lq4jwpbI3qBdAltU5mfoKsqlxmZhjRI1ViaAxv4ibI2Bw/640?wx_fmt=png&from=appmsg#imgIndex=23)
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/ZRhjO8xAWr6TS4SWxgh7ibyC0xAu4nXrcLhxsMLXHwRKWdcN1UI7RbUkPIjs0GXfITNLt8xZuZ9CmHbGgOzVJVe4RNlLpKk8Mmjq19jtHt1E/640?wx_fmt=png&from=appmsg#imgIndex=24)
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/ZRhjO8xAWr6oicp45OF89RT6ibPpLAkkFmnLzvPa62e5fZhSHcmtTop2J5icjZj5iaG3Qwgww4cRdTQ8MdTzYkDajHq2rNH6mYLHRWnwAoOA3tc/640?wx_fmt=png&from=appmsg#imgIndex=25)
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/ZRhjO8xAWr77tMZhFyT3DyUL21Pyz6qZ48ByTKrbYPYYO65b1x3xKDRGVCiaYNMYru15a7OS6WUCvibBSxhLTI4CflTdblrugnwyicxqn1CCfQ/640?wx_fmt=png&from=appmsg#imgIndex=26)
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/ZRhjO8xAWr7a3V0nCQORBlq0szFM7kwptqQmPlIrEs4oIxT5llZpIfMUticqq8ITFlpyMkn48U8wgPepnpKhUIUdQ0mVORFIWx48icVf33EMM/640?wx_fmt=png&from=appmsg#imgIndex=27)
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/ZRhjO8xAWr4dCBevmcRULciaurdkypqAc2JfbxMMjcjyec9IsOjF1GBkoAPxgTeoqmCoibaicQvt65tibLIzf6WibWo055RLbpu6n1sTR1URBkiaE/640?wx_fmt=png&from=appmsg#imgIndex=28)
...
\[3\] GitHub, “About GitHub Copilot Memory,” GitHub Docs. \[Online\]. Available: [https://docs.github.com/en/copilot/concepts/agents/copilot-memory](https://docs.github.com/en/copilot/concepts/agents/copilot-memory)

\[4\] Anthropic, “Manage Claude’s Memory,” Claude Code Docs. \[Online\]. Available: [https://code.claude.com/docs/en/memory](https://code.claude.com/docs/en/memory)
...
\[6\] B. Ahmeti, M. Linder, R. Groner, and R. Wohlrab, “Architecture Decision Records in Practice: An Action Research Study,” Zenodo, 2024. \[Online\]. Available: [https://zenodo.org/records/11635100](https://zenodo.org/records/11635100)

\[7\] C. Packer, S. Wooders, K. Lin, V. Fang, S. G. Patil, I. Stoica, and J. E. Gonzalez, “MemGPT: Towards LLMs as Operating Systems,” arXiv:2310.08560, 2023. \[Online\]. Available: [https://arxiv.org/abs/2310.08560](https://arxiv.org/abs/2310.08560)

\[8\] T. R. Sumers, S. Yao, K. Narasimhan, and T. L. Griffiths, “Cognitive Architectures for Language Agents,” *Transactions on Machine Learning Research*, 2024. \[Online\]. Available: [https://arxiv.org/abs/2309.02427](https://arxiv.org/abs/2309.02427)

\[9\] D. Wu, H. Wang, W. Yu, Y. Zhang, K.-W. Chang, and D. Yu, “LongMemEval: Benchmarking Chat Assistants on Long-Term Interactive Memory,” in *Proc. ICLR*, 2025. \[Online\]. Available: [https://arxiv.org/abs/2410.10813](https://arxiv.org/abs/2410.10813)

\[10\] M. N. Uddin, K. Shubham, E. Blanco, C. Baral, and G. Wang, “From Recall to Forgetting: Benchmarking Long-Term Memory for Personalized Agents,” in *Findings of ACL*, 2026. \[Online\]. Available: [https://arxiv.org/abs/2604.20006](https://arxiv.org/abs/2604.20006)
