---
title: "Claude Code神级技巧：让AI编程更省心"
source: "https://mp.weixin.qq.com/s/cHs53TE4MvCqnyS5lao3KA"
author:
  - "[[陈尧章]]"
published:
created: 2026-08-21
...
tags:
  - "clippings"
---
> [!summary] 阅读总结（2026-08-21 整理）
> **核心论点**：AI 编程工具的门槛已经降下来了，差距在于「会不会用」。全文主线是**把规则和操作前置固化，而不是每次对话现说**——既省 Token 又省心。

## 阅读总结

### 1. CLAUDE.md 该写什么

反直觉的结论：**别写技术栈**（Claude 自己读代码就能推断，白占上下文）。该写的是「AI 推断不出来、又反复影响协作」的规则。作者的三条规则都来自实际踩坑：

| 规则 | 起因 |
|------|------|
| 不确定就问我，别自作主张 | 重构时 AI 发现跨目录依赖却选择跳过，导致返工 |
| 提问前先自己查代码 | 对上一条的补丁，避免 AI 问出翻翻代码就能答的问题 |
| 标注「人工负责」的边界 | `.proto` 只改源文件，`.pb.go` 由 protoc 生成；Bazel 项目别跑 `go build` |

**什么时候该写进去**——三个信号：AI 第二次犯同样的错、Review 中发现它本该懂的项目规范、你发现自己在重复上次会话说过的话。

**分层加载**：全局 `~/.claude/CLAUDE.md` 放通用工作习惯，项目级 `CLAUDE.md` 跟仓库走；规则多了就迁到 `.claude/rules/`，按 `paths` 匹配**按需加载**（渐进式披露），支持子目录递归组织，大项目里各团队维护各自规则互不干扰。

### 2. Hook：从「概率执行」到「强制执行」

全文最有价值的一节。CLAUDE.md 里的规则本质是**建议**——长对话中 AI 注意力被当前任务占据就会忘；Hook 是系统层面的**确定性**保障。

- `PostToolUse` + Bazel：文件增删后自动跑 `gazelle` 更新 `BUILD.bazel`（脚本两层过滤：先判断是否 `rm`，再判断路径是否在监听范围）
- `PreToolUse` 拦截 `git add / commit / push`，不让 AI 碰提交

一句话：**行为规范放 CLAUDE.md，确定性操作放 Hook**。前者约束「怎么思考」，后者保证「一定会做」或「一定不做」。

### 3. 降噪哲学：子代理 vs 分支会话

分界线在**要不要继承主会话上下文**：

| | 子代理（Subagent） | 分支会话（`/branch`） |
|---|---|---|
| 上下文 | 不继承主会话 | 继承全部 |
| 适用 | 只要结论不要过程（评估改动影响面） | 依赖前文、用完即弃（生成接口文档、试探不确定方案） |
| 额外收益 | 可路由到更便宜的模型（Haiku / Explore） | 方案不可行直接丢弃，不污染主线 |

### 4. Skill 的两个妙用

- **Prompt 快捷键**：把高频 prompt 封装成斜杠命令。作者的 `/build-commit-msg` 用了三个关键配置——`context: fork`（子代理跑，`git diff` 大段输出不污染主会话）、`agent: Explore`（轻量便宜模型）、`disable-model-invocation: true`（只做「看 diff → 输出命令」一件事）
- **`` ! `` 内联 Bash**：Skill 加载时命令就已执行完，结果直接注入上下文。省掉「想命令 → 等结果 → 再想下一步」的多轮工具调用和中间思考，Token 和响应速度都明显改善

### 5. Token 消耗

底层自带：Auto-Compaction 自动压缩、按行范围读文件 / 差量编辑（`old_string → new_string`）、Prompt Caching。这些开箱即用，叠加上面的实践技巧才是综合效果。

### 6. 团队共享：私有插件市场

「市场」就是个普通 Git 仓库——`.claude-plugin/marketplace.json` 声明有哪些插件，`plugins/<插件名>/` 下放各自的 Skill / MCP / Hook。成员两条命令装完即用（`/plugin marketplace add` + `/plugin install`），`git pull` 就是更新机制，无需构建、无需搭服务。

### 7. 短板

- IDE 集成深度不如 CodeBuddy（无内联 diff 预览），靠官方 JetBrains / VS Code 插件的「Send to Claude Code」补救
- 只能用 Claude 系列模型，不支持切到 DeepSeek 等其他模型

### ⚠️ 一处需要留意的错误

**2.2 节 PreToolUse 配置里的 `if` 字段并非官方 hook 语法。** 官方 hook 配置只有 `matcher`（匹配工具名）和 `hooks[].type` / `command`，没有 `if`。命令级过滤有两条正路：

1. 在脚本里做——读 stdin 的 JSON 判断命令内容（**作者自己 2.1 节的 gazelle 脚本才是正确写法**）
2. 更省事：直接用 `settings.json` 的 `permissions.deny` 写 `Bash(git push:*)`

其余机制（rules 目录、`/branch`、`context: fork`、插件市场）与 2026 年 8 月的实际版本都对得上。

---

## 原文

陈尧章 腾讯云开发者 *2026年8月5日 08:45*
...
感兴趣的同学可以看看 官方文档，很多重复性的手动操作都可以通过 Hook 来自动化。 [https://code.claude.com/docs/en/hooks](https://code.claude.com/docs/en/hooks)
...
此外，子代理还有一个好处—— **可以使用更便宜的模型来处理简单任务** 。根据 官方文档（ [https://code.claude.com/docs/en/sub-agents）](https://code.claude.com/docs/en/sub-agents%EF%BC%89) 的说明，Claude Code 的一些内置子代理本身就配置了较低成本的模型（比如 Haiku），用户也可以自定义子代理并指定模型。这意味着不是所有任务都需要用最贵的 Opus 来跑，简单的代码搜索、文档查询等操作可以自动或手动路由到更便宜的模型上，进一步节省 Token 开销。
...
根据 官方文档（ [https://code.claude.com/docs/en/costs）](https://code.claude.com/docs/en/costs%EF%BC%89) 的说明，Claude Code 在节省 Token 方面有这些底层设计：
