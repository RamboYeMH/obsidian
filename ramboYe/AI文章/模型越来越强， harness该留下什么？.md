...
删掉团队 harness 框架 61% 内容之后的一些思考：什么会被模型内化，什么永远不会，以及在开源框架遍地的今==天，团队自己做 harness 的价==值在哪。
...
- The new rules of context engineering for Claude 5 generation models，Thariq Shihipar，2026-07-24 —— 删掉 80% 系统提示词、六个转变、规则→判据的 before/after 对照
	（ [https://claude.com/blog/the-new-rules-of-context-engineering-for-claude-5-generation-models）](https://claude.com/blog/the-new-rules-of-context-engineering-for-claude-5-generation-models%EF%BC%89)
- Prompting Claude Opus 5（Claude Platform Docs）—— 验证指令要删除而非改写、字面执行导致召回下降、legacy harness scaffolding
	（ [https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/prompting-claude-opus-5）](https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/prompting-claude-opus-5%EF%BC%89)
- Prompting best practices（Claude Platform Docs）—— 过度提示导致过度触发、CRITICAL 措辞降级、prefill 从 API 移除
	（ [https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/claude-prompting-best-practices）](https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/claude-prompting-best-practices%EF%BC%89)
- Agent Harness Design: 3 Patterns for Harnessing Claude's Intelligence，2026-04-02 —— 三个设计模式、what stays / what goes
	（ [https://claude.com/blog/harnessing-claudes-intelligence）](https://claude.com/blog/harnessing-claudes-intelligence%EF%BC%89)
- Harness design for long-running application development，2026-03-24 —— 「每个组件都编码了一个假设」、context resets 成为 dead weight、evaluator 价值边界外移
	（ [https://www.anthropic.com/engineering/harness-design-long-running-apps）](https://www.anthropic.com/engineering/harness-design-long-running-apps%EF%BC%89)
- A harness for every task: dynamic workflows in Claude Code，2026-06-02 —— agentic laziness / self-preferential bias / goal drift
	（ [https://claude.com/blog/a-harness-for-every-task-dynamic-workflows-in-claude-code）](https://claude.com/blog/a-harness-for-every-task-dynamic-workflows-in-claude-code%EF%BC%89)
- Lessons from building Claude Code: How we use skills，2026-06-03 —— 复述默认行为的 skill 零价值、验证类 skill 影响最大
	（ [https://claude.com/blog/lessons-from-building-claude-code-how-we-use-skills）](https://claude.com/blog/lessons-from-building-claude-code-how-we-use-skills%EF%BC%89)
- Skill authoring best practices（Claude Platform Docs）—— 上下文窗口是公共资源、自由度分级、先建评测
	（ [https://platform.claude.com/docs/en/agents-and-tools/agent-skills/best-practices）](https://platform.claude.com/docs/en/agents-and-tools/agent-skills/best-practices%EF%BC%89)
- Effective context engineering for AI agents，2025-09-29 —— 注意力预算、context rot
	（ [https://www.anthropic.com/engineering/effective-context-engineering-for-ai-agents）](https://www.anthropic.com/engineering/effective-context-engineering-for-ai-agents%EF%BC%89)
...
- Harness engineering: leveraging Codex in an agent-first world，2026-02-11 —— 强制不变量而非微管理实现、AGENTS.md 约 100 行的「地图」、用 linter 与 structural test 强制
	（ [https://openai.com/index/harness-engineering/）](https://openai.com/index/harness-engineering/%EF%BC%89)
- GPT-5.5 prompt guidance（OpenAI 开发者文档）—— 删除分步流程指引、过度指定缩小搜索空间、真正的不变量 vs 判断题、从最小 prompt 重新起步
	（ [https://developers.openai.com/api/docs/guides/prompt-guidance?model=gpt-5.5）](https://developers.openai.com/api/docs/guides/prompt-guidance?model=gpt-5.5%EF%BC%89)

Addy Osmani（Google Chrome 团队工程主管，个人博客）

- Agent Harness Engineering，2026-04-19 —— harness 不是缩小而是迁移、棘轮法
	（ [https://addyosmani.com/blog/agent-harness-engineering/）](https://addyosmani.com/blog/agent-harness-engineering/%EF%BC%89)
- Own the Outer Loop，2026-07-15 —— 只有人能继承后果、Verdict / Answerability / Accountability
	（ [https://addyosmani.com/blog/own-the-outer-loop/）](https://addyosmani.com/blog/own-the-outer-loop/%EF%BC%89)
- Earning taste and judgment，2026-07-14 —— 监督者悖论
	（ [https://addyosmani.com/blog/earning-judgment/）](https://addyosmani.com/blog/earning-judgment/%EF%BC%89)
- Software Factories, Light and Dark，2026-07-20 —— back pressure、验证才是瓶颈
	（ [https://addyosmani.com/blog/software-factories/）](https://addyosmani.com/blog/software-factories/%EF%BC%89)
- Agentic Autonomy Levels，2026-07-02 —— 六级自主度、三个判定问题、四个反模式
	（ [https://addyosmani.com/blog/agentic-autonomy-levels/）](https://addyosmani.com/blog/agentic-autonomy-levels/%EF%BC%89)
- The Intent Debt，2026-06-05 —— 技术债 / 认知债 / 意图债的区分
	（ [https://addyosmani.com/blog/intent-debt/）](https://addyosmani.com/blog/intent-debt/%EF%BC%89)
- Stop Using /init for AGENTS.md，2026-02-23 —— 能找到就删、第二个事实来源、还没修的摩擦
	（ [https://addyosmani.com/blog/agents-md/）](https://addyosmani.com/blog/agents-md/%EF%BC%89)
- Agent Skills，2026-05-03 —— 流程优先于散文
	（ [https://addyosmani.com/blog/agent-skills/）](https://addyosmani.com/blog/agent-skills/%EF%BC%89)
- The Orchestration Tax，2026-05-24 —— 启动便宜、收口昂贵
	（ [https://addyosmani.com/blog/orchestration-tax/）](https://addyosmani.com/blog/orchestration-tax/%EF%BC%89)
- Comprehension Debt，2026-03-14 —— 认知债滋生虚假信心
	（ [https://addyosmani.com/blog/comprehension-debt/）](https://addyosmani.com/blog/comprehension-debt/%EF%BC%89)
...
> 2026-08-11 实践记录。用文章的判据逐条扫了自己的 harness，做了改造并实测。
> 下面每个数字都是跑出来的，不是估的。
...
|---|---:|---|
| `CLAUDE.md` | 5,153 | 🔴 常驻 |
| `memory/MEMORY.md` | 1,631 | 🔴 常驻 |
| `code-skill/SKILL.md` | 13,272 | 按需 |
| `online-logs/SKILL.md` | 9,333 | 按需 |
| `obs/SKILL.md`（全局） | 4,842 | 按需 |
| `switch-branch/SKILL.md` | 3,707 | 按需 |
| `obs/SKILL.md`（项目） | 2,902 | ⚠️ 被静默覆盖，从不加载 |

## 二、扫出来的四个病症

**1. 同一份代码规范存在三份**（对应文章「第二个事实来源」）

CLAUDE.md 的 13 条 Code Standards、MEMORY.md 的 Code Conventions、code-skill 的 21 条——高度重叠但措辞不一致：CLAUDE.md 说「必须用 Get 方法」，规则 15 说「自动生成 GetXXX/SetXXX」，MEMORY.md 又补了「nil 时初始化再返回」。读到分歧时得自己裁决信哪份。

**2. 常驻区大半是「读代码就能找到」的内容**（对应 Addy 的删除判据）

Server Nodes 表、Shared Packages 说明、init pattern——`ls` + 读 Makefile 全都有。而且**已经腐化了**：CLAUDE.md 把 `rank/ → bin/rankserv` 列为正式节点，Makefile 里那行实际是注释掉的（`platform/` 同样）。文档说有个排行榜服务在跑，构建列表里根本没有。这不是理论风险，是已发生的事实。

覆盖也不全：40 个顶层目录只描述了 17 个，`bilog/`、`gmt/`、`igame/`、`moniter/` 一个字没提——而当前正在改的文件里就有 `ctl_bilog.go`。

**3. 工具层根本不存在**（对应文章第五关）

`.golangci.yml` 是 2021-05-28 建仓次日从 golangci-lint **官方示例**整份拷来的，五年零改动。里面还留着别人项目的路径（`local-prefixes: github.com/golangci/golangci-lint`、`skip-dirs: pkg/golinters/...`——本仓库没这些目录）。而 `golangci-lint` 和 `golint` **都没装**，`make check` 里那行是注释掉的。

所以 `dupl`/`lll` 出现在 disable 列表里不是决策，是模板默认值。**所有约束 100% 押在指令层**，这解释了规则清单为什么只涨不跌。

**4. 两个同名 obs skill，根目录定义互相冲突**

项目版把 `指尖王国-游戏开发/` 当 vault 根，全局版把 `ramboYe/` 当根并警告别把技术笔记写进游戏库。实地核对：全局版对，项目版的 `00-总览.md` 也已过期（实际是 `总览.md`）。同名 + 静默覆盖 + 无任何报错，是最难发现的一类重复。

## 三、做了什么

**建工具层**（golangci-lint v2.12.2 + `.golangci.local.yml` + `scripts/check-local.sh`，全部走 `.git/info/exclude`，不入版本库）

实测数据：

| 模式 | 告警数 | 可用性 |
|---|---:|---|
| 全量 `game/wmap/internal` | **5,217** | ❌ 没人会看，等于没有门禁 |
| 增量（未提交改动） | **15** | ✅ 人能判 |

它当场抓出了指令层没兜住的东西：

- `dupl` **22 处真实重复**，如 `ctl_battlefield_game_play.go:633-675` ≡ `ctl_gather.go:264-306`——规则 5「禁止复制粘贴」写了这么久没拦住
- `revive` 6 个新写的导出方法缺注释（`Mgr.GetMerchantFleetData` 等）——规则 21 同理
- 顺带发现 `game/wmap/draw/ui` 依赖 `gxui` 而 vendor 里没有、编译不过，是个死包
...
|---|---:|---:|
| 常驻区 | 6,784 | **4,821（−29%）** |
| 代码规范的事实来源 | 3 份，措辞冲突 | **1 份** |
| CLAUDE.md | 5,153 | 3,877 |
| 工具层 | 不存在 | 增量可跑，exit code 可判 |

## 四、实践中对文章的四点修正

**1. 「能找到就删」有边界：能找到 ≠ 会想到去找**

实测：不看 CLAUDE.md，两条命令就能还原出比它更准的节点表。但有一类**还原不出来**——哪些目录是死的（`opencode_v1/`、`tempssss/`）、主战场是哪个、`igame/` 和 `game/` 分工、以及「GVG」「火车」「毒地图」这些黑话。

所以该删的是**描述性**内容（「它是什么」），该留的是**地图性**内容（「往哪看、什么还活着、这个词什么意思」）。OpenAI 说的 "a map, with pointers"——地图有价值，百科全书没有。而项目原本写的恰恰全是百科全书，地图一条没有。

**2. 去重不一定让文件变小，别拿字数当 KPI**

code-skill 瘦身后从 13,272 **涨到** 14,747。因为 MEMORY.md 里有两条它完全没有的内容（Get 方法内 nil 初始化、`accessor_XXX.go` 文件规范），必须先迁移再删。真正该看的指标是**常驻区大小**和**事实来源数**，不是总字数。

**3. 能沉降到工具层的规则比预想的少得多**

21 条里真正能搬走的只有 1.5 条（规则 5 完整、规则 21 的导出类型部分）。`lll` 管不到「参数超 3 个即使不超长也要换行」，case 换行和 `rt` 命名没有现成 linter。

**所以工具层的价值不在于替指令层减负**，而在于——它是**验收标准唯一能落地的载体**。没有它，「什么算改完了」这件事根本无法表达。这也解释了为什么四层归属表里 `L3·验收标准` 是空的：不是忘了写，是没地方写。

**4. 存量代码库只能用增量模式**

文章没讲这一点，但对任何有历史的项目都是决定性的。5,217 条存量下，全量门禁必然被无视——正对应文章那句「一个人判不动的 gate 等于没有 gate，而且更糟，它制造了『已经审过』的假象」。

## 五、还欠着的（全是 L3，只有人能写）

- [ ] `rank/`、`platform/` 到底停用了没——是 Makefile 漏了还是服务下线了，文档和构建脚本互相矛盾
- [ ] 黑话表：GVG / 火车 / 毒地图 / 神庙……不在任何语料和代码注释里
- [ ] `igame/` 与 `game/`、`actv/` 与 `actv_com/` 的分工
- [ ] CLAUDE.md 补「验收」段和「不可逆动作边界」段
- [ ] 给留下的规则逐条补「它防的是哪个具体失败」——否则下个模型代际还是不敢删
- [ ] pre-push hook（要挂在 `.git/hooks/` 本地，Makefile 是追踪文件不能动）

> **一句话**：文章说 harness 的长期价值等于「只有你们知道」和「只有人能承担」这两处的面积。
> 这轮改造真正做的事，是把占了大半篇幅的 L0/L1 清出去，腾出位置——而那两处的面积，目前还基本是空白。
